                        ...
                        <div class="modal-content">
                            <form action="{{ route('admin.Partner.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="modal-body">
                                    <div class="mb-3">
                                        <label for="partnerImage" class="form-label">Partner Image</label>
                                        <input type="file" class="form-control" id="partnerImage" name="image" accept="image/*">
                                    </div>
                                    <div class="mb-3">
                                        <label for="partnerTitle" class="form-label">Partner Title</label>
                                        <input type="text" class="form-control" id="partnerTitle" name="title" required>

                                    </div>

                                    <div class="mb-3">
                                        <label for="partnerDesc" class="form-label">Partner Description</label>
                                        <input type="text" class="form-control" id="partnerDesc" name="description">

                                    </div>

                                    <div class="mb-3">
                                        <label for="partnerLink" class="form-label">Partner link</label>
                                        <input type="text" class="form-control" id="partnerLink" name="link">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div class="modal fade" id="editBrandModal{{ $item->id }}" tabindex="-1" aria-labelledby="editBrandModalLabel{{ $item->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <form action="{{ route('admin.Partner.edit', $item->id) }}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            @method('PUT')
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editBrandModalLabel{{ $item->id }}">Edit Partner</h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                ...
                                                @endif
                                                <div class="mb-3">
                                                    <label for="editBrandName{{ $item->id }}" class="form-label">Partner Title</label>
                                                    <input type="text" class="form-control" id="editBrandName{{ $item->id }}" name="title" value="{{ $item->title }}" required>
                                                </div>

                                                <div class="mb-3">
                                                    <label for="editBrandName{{ $item->id }}" class="form-label">Partner Description</label>
                                                    <input type="text" class="form-control" id="editBrandName{{ $item->id }}" name="description" value="{{ $item->description }}">
                                                </div>

                                                <div class="mb-3">
                                                    <label for="editBrandName{{ $item->id }}" class="form-label">Partner Link</label>
